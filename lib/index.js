...
// 白名单只保留真正的跨源调用方：chat.deepseek.com 页面的 token 书签。
// 同源页面（DSH web/桌面端任意端口）由 isSameOrigin 放行，无需列入。
const ALLOWED_ORIGINS = ['https://chat.deepseek.com']
...
export function apply(ctx, config = {}) {
  ...
  // 同源判定：页面 Origin 的主机与端口等于请求 Host，则属于同一 DSH 实例
  // （web 版 3080、桌面端 49962、任意端口都放行），无需 CORS。
  const sameOrigin = (origin, hostHeader) => {
    if (!origin || !hostHeader) return !origin
    try {
  ...
  const originAllowed = (origin, hostHeader) => {
    ...
    if (sameOrigin(origin, hostHeader)) return true
