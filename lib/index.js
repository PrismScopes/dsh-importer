...
/** 硬依赖：fs（落盘）、subprocess（curl 网络层）、timer（自动同步）。 */
export const inject = ['fs', 'subprocess', 'timer']
...
export function apply(ctx, config = {}) {
  ...
  const extractToken = (v) => {
    ...
    return v.token || v.access_token || v.accessToken || null
  }

  const getCwd = async () => fs.processPath(await fs.resolve('.'))
  ...
  const removeFiles = async (names) => {
    try {
      const exe = await subprocess.resolveExecutable('cmd')
      const cwd = await getCwd()
      const handle = subprocess.spawn({
        argv: [exe, '/c', 'del', '/q', '/f'].concat(names),
        cwd: cwd,
        stdio: { stdin: 'ignore', stdout: { maxBytes: 4096 }, stderr: { maxBytes: 4096 } },
        graceMs: 5000
      })
      await handle.done
    } catch (e) { ctx.logger.warn(`[${name}] 删除文件失败: ${String(e)}`) }
  }
  ...
  const saveSessionData = async (sessionId, payload) => {
    try {
      const target = await fs.resolve('dsweb-sessions-' + String(sessionId) + '.json')
  ...
  const initImport = async () => {
    ...
    try {
      ...
      await extract.done
      const target = await fs.resolve('conversations.json')
  ...
  const api = {
    ...
    'buildPrompt': async (args) => {
      ...
      if (!sid) return { ok: false, error: '缺少 sessionId' }
      try {
        const target = await fs.resolve('dsweb-sessions-' + String(sid) + '.json')
    ...
    'readSession': async (args) => {
      ...
      try {
        const target = await fs.resolve('dsweb-sessions-' + String(sid) + '.json')
        return { ok: true, data: JSON.parse(await fs.readText(target)) }
      } catch (e) { return { ok: false, error: String(e) } }
    },
  ...
  maybeAutoInit()
  ctx.logger.info(`[${name}] host ready（自动同步 + Origin 校验 + 数据沿用 dsweb-* 文件）`)
}
