...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    // 书签在 chat.deepseek.com 页面执行，URL 必须在生成时就写死为当前 DSH
    // 实例地址（web 版或桌面端任意端口），运行时取 location 会拿到 chat 域名。
    var bookmarkEndpoint = (function () {
      try { return (window.location.origin || 'http://127.0.0.1:3080') + '/api/dsweb/token' } catch (e) { return 'http://127.0.0.1:3080/api/dsweb/token' }
    })()
        ...
        (props) => {
          ...
          const helpEl = React.createElement('div', { className: 'dsw-card' },
            ...
            React.createElement('div', { className: 'dsw-help' },
              ...
              React.createElement('code', { className: 'dsw-code' },
                "javascript:(function(){try{var r=localStorage.getItem('userToken');var v=JSON.parse(r);var x=v&&v.value!==undefined?v.value:v;var t=typeof x==='string'?x:(x.token||x.access_token||null);if(!t){alert('未登录');return}fetch('" + bookmarkEndpoint + "',{method:'POST',headers:{'Content-Type':'text/plain'},body:JSON.stringify({token:t,source:'bookmark'})}).then(function(r){return r.json()}).then(function(d){alert('配置'+(d.ok?'成功':'失败'))}).catch(function(e){alert('配置失败')})}})();")
            )
          )
